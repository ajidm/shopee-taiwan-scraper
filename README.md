...
Implikasi: ini mengeliminasi hipotesis bahwa tooling stealth kita (`rebrowser-playwright` + `puppeteer-extra-plugin-stealth`) yang dikenali/di-fingerprint secara spesifik — errornya sama persis walau tooling anti-deteksi itu dilepas total. Kode `90309999` pada endpoint-endpoint personalisasi/rekomendasi ini kemungkinan besar bukan murni sinyal anti-bot adversarial, melainkan **kebijakan produk**: Shopee menggerbang endpoint yang butuh identitas user (rekomendasi personal, flash sale yang terikat akun) di balik login untuk *semua* sesi guest — bukan cuma yang dicurigai bot — sementara endpoint non-personal seperti `get_pc`/`get_rw` (target utama tugas ini) berpotensi punya perlakuan berbeda. Ini belum sepenuhnya dikonfirmasi untuk `get_pc` di `shopee.co.id` secara langsung (belum sempat mendapat `shopId`/`itemId` produk nyata dari endpoint yang tidak tergerbang, dalam waktu yang tersedia), tapi memberi arah baru yang lebih presisi untuk investigasi lanjutan: uji `get_pc` langsung (bukan lewat endpoint personalisasi) di region mana pun yang bisa diakses.
