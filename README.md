...
This also means **this technique has not been validated on `shopee.tw`** at all (blocked by the account-region limitation noted throughout this document) — what's confirmed is: two real successes happened once, on `shopee.co.id`, under conditions this project can no longer cleanly reproduce on the same account. Given the risk of further degrading the only available test account, no further live testing was pursued on it beyond this point.
