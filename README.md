...
> **Status:** Code is complete, type-check/lint/build are clean, and it **has proven to work end-to-end twice**, both on `shopee.co.id` — via a full realistic navigation chain (homepage → search → click product) and, separately, via a deterministic click on an injected link to a pre-specified product ID, both from a genuine, logged-in persistent Chrome profile (see "Breakthrough" in [Known Limitations](#known-limitations)). **Important:** subsequent attempts to reproduce this through the integrated codebase (not a standalone script) both failed on fresh items, even after adding an organic-interaction warmup step — the current best explanation is that the two successes reflected a temporary, freshly-cooled-down account risk state rather than a technique that reliably reproduces on demand (see the "revised interpretation" note in Known Limitations). Earlier in development, the two original `shopee.tw` sample items got repeatedly flagged by Shopee's anti-bot system (`/verify/traffic/error`, code `90309999`) after volume testing, which turned out to be persistent across IP/network/proxy/login/browser-binary/headless-mode/CDP-patch-depth (8 independent variables systematically eliminated — see [Methods & Experiments Tried](#methods--experiments-tried) and [Known Limitations](#known-limitations)). Two independent analysis reviews (see [External Analysis & Further Fixes](#external-analysis--further-fixes)) produced further improvements: granular error classification, per-product circuit breaker, sticky-proxy-per-session (a real bug fixed), in-browser fetch to eliminate TLS mismatch, and a ramp-up load test. This project's honest conclusion: a working, reliable bypass was not achieved and verified as reproducible within the time available; the architecture, error handling, and observability are production-grade, but Shopee's current anti-bot posture — evaluated across nine independent variables plus two real but non-reproduced successes — appears to sit beyond what any combination of publicly documented client-side techniques can reliably defeat.
...
**Summary of eight independent variables eliminated**, all producing the identical `error: 90309999`:
...
| 8 | Persistent profile vs. `storageState` snapshot | `PERSISTENT_PROFILE=true` with a genuine login done directly inside a fresh persistent profile (not exported/imported cookies) — see the "device-cookie binding" analysis below | Still failed |
...
The third row is a genuine gap between the two prior attempts — `PERSISTENT_PROFILE=true` already exists in `session.manager.ts`, but has never actually been tested with a genuine login inside it and reused over time (rather than used once).

**Update — tested (8th confirmation): a genuine persistent profile still fails.** A dedicated persistent Chrome profile (a fresh, empty profile directory — `patchright` was used to launch it, sidestepping a known `playwright-extra` bug where `launchPersistentContext` ignores a custom `executablePath`) was logged into manually, once, directly inside that exact profile directory (not via `storageState` export/import — the login session lives natively in the profile's own disk state). It was then reused via `PERSISTENT_PROFILE=true` against the same `shopee.co.id` product.

Result: **`get_pc` and `get_rw` both still returned `error: 90309999`.** This weakens the "persistent profile vs. storageState snapshot" framing of the device-cookie-binding hypothesis — at least for a profile that was *just* created, with no real usage history/"aging." Two readings remain open: (a) the real differentiator isn't persistent-vs-snapshot at all, but genuine long-term usage history that a freshly-made profile still lacks even though it's technically persistent, or (b) device-cookie binding isn't quite the right frame, and the root cause remains CDP/automation control itself, independent of how the cookies are held. Given the account-risk concerns already noted above, further iterations on "age" a persistent profile with real, spaced-out usage over days/weeks were not pursued in this session.
...
### Breakthrough: a real, live success via the full realistic navigation chain

Roughly a day after the CAPTCHA escalation noted above (enough time for the account's risk score to plausibly cool down), the "realistic navigation chain" angle that had been interrupted earlier was retried — this time using the same persistent profile from the test above (already logged in, no longer freshly-created), combined with **genuine click-based navigation instead of `page.goto()`**: homepage → click the search box → type a keyword → press Enter → scroll → click an actual product link in the results, rather than constructing/navigating to a product URL directly.
...
That said, this is a concrete, reproducible, positive result, not a hypothesis — and it's consistent with two independent signals already documented above: the external analysis's suggestion that navigation sequence/state matters (see "ruling out the signature-replay hypothesis" above), and the reference projects' emphasis on a genuinely trusted, aged browser profile (see "device-cookie binding" above). The practical implication for a production strategy: prefer driving Shopee's own front-end through realistic, click-based navigation from a long-lived, genuinely-used authenticated profile, rather than deep-linking directly into product URLs from a fresh session — closer to how `shopee-mcp` and the two Selenium-based reference scrapers already operate, and a meaningfully different architecture from this project's original design (which deep-links directly via `page.goto()` to `https://shopee.tw/a-i.{storeId}.{dealId}` for speed and simplicity).

This success was on `shopee.co.id`, not the task's actual `shopee.tw` target (see the earlier note on why — Shopee accounts are region-specific and only an Indonesian account was available), so it validates the *approach* rather than proving it works on the specific target domain. Given the account-risk concerns already discussed, this specific account/profile was not pushed further (e.g. repeating the chain immediately against more products) to avoid risking another escalation.

**Isolating the actual variable: click vs. `goto()`, made deterministic and API-compatible.** The successful run above still has a structural mismatch with the task's API contract: it reached a product by *organically searching and clicking whatever appeared first* — but the API must serve an arbitrary, caller-specified `storeId`+`dealId`, and Shopee's search only takes keywords, not product IDs. So the open question was whether "a genuine click" specifically was the differentiator (as opposed to the search/discovery process itself, the profile's slight aging, or the elapsed time) — and if so, whether that could be reproduced for a **deterministic**, pre-known target instead of whatever a search happens to surface.

This was tested directly: a fresh product ID was first discovered via search **without clicking it** (just reading the link's `href`), then — in a separate, reset navigation back to the homepage — a plain `<a>` element pointing at that *exact, already-known* product URL was injected into the page's DOM via `page.evaluate()`, and clicked using Playwright's `.click()` (which dispatches trusted, CDP-level input events — categorically different from `page.goto()`, which is closer to typing a URL directly with no in-page event at all).

**Result: success again**, on a second, different fresh product (`Powerbank Xm PLM02Z 10000 mAh`, `shopId=1651787050`, `itemId=44082009327`) — chosen deterministically ahead of time, not discovered through the click itself. This is the key result: **a genuinely clicked navigation to an arbitrary, pre-specified product succeeds, without needing organic search/discovery** — which is directly compatible with the API's `GET /shopee?storeId=&dealId=` contract.

This is now implemented as a proper technique — `NAVIGATION_STRATEGY=click` (see `src/techniques/clickNavigation.ts`): land on the homepage, inject a link to the exact requested product URL, and click it instead of `page.goto()`-ing there directly. It was tested against a guest session (no login, no persistent profile) targeting the task's actual `shopee.tw` domain and one of the two original, heavily-flagged sample items — which still returned `error: 90309999`, unsurprising given how many times that specific item has been hit throughout this project's testing (see "Methods & Experiments Tried" above).

**Update — replication attempts through the integrated codebase failed, correcting the "click alone is sufficient" framing.** To validate the technique end-to-end (not just in a standalone script), it was wired into `session.manager.ts` and tested via the actual live server, hit through the public ngrok URL — first against a fresh, never-touched item (`Apex Keyboard Mekanik Gaming`, `shopId=1744608100`, `itemId=46958749320`): **failed**, `error: 90309999` on the very first attempt. A short organic-interaction step was then added (`warmupWithOrganicSearch()` in `clickNavigation.ts` — typing a jittered, unrelated search keyword, pressing Enter, waiting for results, scrolling — before the injected click), reasoning that the two earlier successes always had a real search interaction happen in the same page moments before the click, whereas this integrated version went straight from homepage to injected-click with zero prior activity. Re-tested against a second fresh item (`Robot Workmate III Mechanical Gaming Keyboard`, `shopId=131736292`, `itemId=40018658969`) with the organic warmup in place: **also failed**, `error: 90309999` again.

**Revised interpretation.** Neither "a genuinely clicked event" nor "clicking after organic interaction" reproduced the earlier success on retry. The most parsimonious explanation, given the full chronology: the two successes happened in a narrow window right after the account's CAPTCHA escalation had ~a day to cool down — and every automated test run against that same account/profile since then (including these two failed replications, plus an interleaved discovery script and a repeat of an already-burned item) has likely added more suspicious signal to that specific account, degrading whatever "clean" standing produced the two successes, regardless of which navigation technique is used. In other words: the successes may reflect a **temporary account/session state** (freshly cooled-down risk score) rather than a **technique** that reliably reproduces on demand — click-navigation and the organic-warmup step remain in the codebase as reasonable, low-cost things to try (there's no evidence they hurt), but this project cannot currently claim they are *the* fix, only that they were present during two real successes whose conditions have not been cleanly isolated from the account's state at the time.

This also means **this technique has not been validated on `shopee.tw`** at all (blocked by the account-region limitation noted throughout this document) — what's confirmed is: two real successes happened once, on `shopee.co.id`, under conditions this project can no longer cleanly reproduce on the same account.

**Final confirmation — retrying the exact same previously-successful item also failed.** As a last check, the *exact* item that succeeded in the first breakthrough (`GOOJODOQ Wireless Bluetooth 5.3 Headphones`, `shopId=1412347694`, `itemId=46755193498`) was requested again through the live server, using the identical ingredients that produced the original success — the same persistent, logged-in Chrome profile, the same `patchright` engine, both `direct` and `click` navigation strategies (via `NAVIGATION_STRATEGY=auto`). **It failed too**, `error: 90309999` on every attempt. Since this isn't a case of a fresh item getting "burned" by repeated testing (it had only been touched once, successfully, the day before) and every technical ingredient was identical to the successful run, this is the strongest evidence yet that the differentiator was never the technique itself — it was the account/session's risk-score state at that specific moment, which has since shifted (most plausibly degraded further by the volume of automated testing this project ran against the same account in between, including this very confirmation check). Given the risk of degrading the only available test account even further, no additional live testing was pursued on it beyond this point.
