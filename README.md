...
Ini bukan lagi murni pertanyaan "bagaimana menghindari deteksi bot", melainkan indikasi bahwa **akses guest/anonim ke Shopee saat ini sangat dibatasi secara umum** — sebuah keputusan produk/kebijakan yang levelnya di atas apa pun yang bisa diatasi lewat teknik anti-deteksi di sisi client. Percobaan mendapatkan `shopId`/`itemId` produk nyata dari `shopee.co.id` untuk menguji `get_pc` secara langsung (di luar endpoint personalisasi) belum berhasil dalam waktu yang tersedia karena wall ini juga menghalangi navigasi ke halaman kategori/pencarian itu sendiri — bukan cuma API pemanggil datanya.
