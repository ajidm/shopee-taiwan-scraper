...
> **Status:** Code is complete, type-check/lint/build are clean, and it **has proven to work end-to-end twice** — most recently via a full realistic navigation chain (homepage → search → click product) from a genuine, logged-in persistent Chrome profile, successfully fetching a complete, real product payload matching the `get_pc` schema (see "Breakthrough" in [Known Limitations](#known-limitations)). Earlier in development, the two original sample items got repeatedly flagged by Shopee's anti-bot system (`/verify/traffic/error`, code `90309999`) after volume testing, which turned out to be persistent across IP/network/proxy/login/browser-binary/headless-mode/CDP-patch-depth (8 independent variables systematically eliminated — see [Methods & Experiments Tried](#methods--experiments-tried) and [Known Limitations](#known-limitations)). Two independent analysis reviews (see [External Analysis & Further Fixes](#external-analysis--further-fixes)) produced further improvements: granular error classification, per-product circuit breaker, sticky-proxy-per-session (a real bug fixed), in-browser fetch to eliminate TLS mismatch, and a ramp-up load test. The recent breakthrough suggests realistic click-based navigation from a trusted, aged, authenticated profile — rather than deep-linking directly into product URLs — is likely the more production-viable architecture; a consistent 200+ item volume test would most likely require this approach combined with items/products that haven't been "burned" by repeated testing.
...
This is now implemented as a proper technique — `NAVIGATION_STRATEGY=click` (see `src/techniques/clickNavigation.ts`): land on the homepage, inject a link to the exact requested product URL, and click it instead of `page.goto()`-ing there directly. It was tested against a guest session (no login, no persistent profile) targeting the task's actual `shopee.tw` domain and one of the two original, heavily-flagged sample items — which still returned `error: 90309999`, unsurprising given how many times that specific item has been hit throughout this project's testing (see "Methods & Experiments Tried" above). **This technique has not yet been validated on `shopee.tw` against a genuinely fresh item, or combined with login/persistent-profile on that domain** (blocked by the same account-region limitation noted throughout this document) — but given it reproduced successfully twice independently on `shopee.co.id`, including once in a fully deterministic, API-compatible form, it's a strong, concrete candidate for the production navigation strategy once a `shopee.tw` account or a fresh, untested `.tw` item becomes available to confirm.
