...
**Konfirmasi final — diuji langsung ke `get_pc` pada produk nyata:** menggunakan `shopId`/`itemId` produk `shopee.co.id` yang valid dan baru (`50248646` / `18482027840`), navigasi langsung ke halaman produk tersebut dari sesi guest yang benar-benar baru (browser baru, tanpa proxy, koneksi langsung) tetap menghasilkan **`get_pc` mengembalikan `error: 90309999`** — response HTTP-nya sendiri `200 OK`, tapi payload JSON-nya berisi kode error yang sama, bukan data produk. Ini pola yang identik dengan endpoint personalisasi dan wall login di `.tw`, kini terkonfirmasi langsung pada endpoint target tugas ini (`get_pc`), bukan cuma dugaan dari endpoint lain. Kesimpulannya: pembatasan ini berlaku merata di seluruh permukaan API Shopee untuk sesi guest baru, termasuk `get_pc`/`get_rw` — memperkuat bahwa akar masalahnya adalah kebijakan akses guest yang sangat ketat saat ini, bukan sesuatu yang bisa diatasi murni lewat teknik client-side yang didokumentasikan di atas.
