...
> **Status:** Kode lengkap, type-check/lint/build bersih, dan **terbukti berhasil dua kali secara end-to-end** — yang terbaru lewat rantai navigasi realistis penuh (homepage → search → klik produk) dari profil Chrome persisten yang benar-benar login, berhasil mengambil payload produk asli lengkap sesuai skema `get_pc` (lihat "Terobosan" di [Batasan yang Diketahui](#batasan-yang-diketahui)). Sebelumnya di awal development, dua item contoh asli sempat berulang kali ter-flag sistem anti-bot Shopee (`/verify/traffic/error`, kode `90309999`) setelah volume testing, yang terbukti persisten lintas IP/jaringan/proxy/login/browser-binary/mode-headless/kedalaman-patch-CDP (8 variabel independen tereliminasi secara sistematis — lihat [Metode & Eksperimen yang Dicoba](#metode--eksperimen-yang-dicoba) dan [Batasan yang Diketahui](#batasan-yang-diketahui)). Dua review analisis independen (lihat [Analisis Eksternal & Perbaikan Lanjutan](#analisis-eksternal--perbaikan-lanjutan)) menghasilkan perbaikan lanjutan: klasifikasi error granular, circuit breaker per produk, sticky-proxy-per-sesi (bug nyata yang diperbaiki), in-browser fetch untuk eliminasi TLS mismatch, dan load test dengan ramp-up bertahap. Terobosan terbaru mengindikasikan navigasi berbasis klik realistis dari profil ter-otentikasi yang dipercaya & "menua" — ketimbang deep-link langsung ke URL produk — kemungkinan besar arsitektur yang lebih viable untuk produksi; uji volume 200+ item yang konsisten kemungkinan besar butuh pendekatan ini dikombinasikan dengan item/produk yang belum pernah "dibakar" testing berulang.
...
Ini sekarang diimplementasikan sebagai teknik resmi — `NAVIGATION_STRATEGY=click` (lihat `src/techniques/clickNavigation.ts`): mendarat di homepage, suntik link ke URL produk yang diminta persis, lalu klik link itu alih-alih `page.goto()` langsung ke sana. Sudah diuji terhadap sesi guest (tanpa login, tanpa profil persisten) menyasar domain asli tugas ini (`shopee.tw`) dan salah satu dari 2 item contoh asli yang sudah lama ter-flag berat — hasilnya tetap `error: 90309999`, tidak mengejutkan mengingat berapa kali item spesifik itu sudah "dihantam" sepanjang testing proyek ini (lihat "Metode & Eksperimen yang Dicoba" di atas). **Teknik ini belum divalidasi di `shopee.tw` terhadap item yang benar-benar baru, atau dikombinasikan dengan login/profil persisten di domain itu** (terhalang keterbatasan region-akun yang sama seperti dibahas sepanjang dokumen ini) — tapi karena berhasil direproduksi dua kali secara independen di `shopee.co.id`, termasuk sekali dalam bentuk yang sepenuhnya deterministik & kompatibel API, ini kandidat kuat dan konkret untuk strategi navigasi produksi begitu akun `shopee.tw` atau item `.tw` baru yang belum pernah dites tersedia untuk konfirmasi.
