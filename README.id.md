...
> **Status:** Kode lengkap, type-check/lint/build bersih, dan **terbukti berhasil dua kali secara end-to-end**, keduanya di `shopee.co.id` — lewat rantai navigasi realistis penuh (homepage → search → klik produk) dan, terpisah, lewat klik deterministik ke link yang disuntik menuju ID produk yang sudah ditentukan sebelumnya, keduanya dari profil Chrome persisten yang benar-benar login (lihat "Terobosan" di [Batasan yang Diketahui](#batasan-yang-diketahui)). **Penting:** percobaan replikasi selanjutnya lewat kode terintegrasi (bukan skrip terpisah) keduanya **gagal** di produk baru, bahkan setelah ditambahkan langkah warmup interaksi organik — penjelasan terbaik saat ini adalah kedua keberhasilan itu mencerminkan kondisi risk-score akun yang sedang "dingin" sesaat, bukan teknik yang reliable direproduksi kapan saja (lihat catatan "revised interpretation" di Batasan yang Diketahui). Sebelumnya di awal development, dua item contoh asli `shopee.tw` sempat berulang kali ter-flag sistem anti-bot Shopee (`/verify/traffic/error`, kode `90309999`) setelah volume testing, yang terbukti persisten lintas IP/jaringan/proxy/login/browser-binary/mode-headless/kedalaman-patch-CDP (8 variabel independen tereliminasi secara sistematis — lihat [Metode & Eksperimen yang Dicoba](#metode--eksperimen-yang-dicoba) dan [Batasan yang Diketahui](#batasan-yang-diketahui)). Dua review analisis independen (lihat [Analisis Eksternal & Perbaikan Lanjutan](#analisis-eksternal--perbaikan-lanjutan)) menghasilkan perbaikan lanjutan: klasifikasi error granular, circuit breaker per produk, sticky-proxy-per-sesi (bug nyata yang diperbaiki), in-browser fetch untuk eliminasi TLS mismatch, dan load test dengan ramp-up bertahap. Kesimpulan jujur proyek ini: bypass yang bekerja dan reliable-direproduksi belum tercapai dan terverifikasi dalam waktu yang tersedia; arsitektur, error handling, dan observability sudah level produksi, tapi postur anti-bot Shopee saat ini — dievaluasi lewat sembilan variabel independen plus dua keberhasilan nyata yang tidak tereproduksi — tampaknya berada di luar jangkauan kombinasi teknik client-side publik mana pun yang bisa diandalkan secara konsisten.
...
**Catatan kenapa `.co.id` dipakai, bukan `.tw`, untuk pengujian login.** Akun Shopee bersifat spesifik per negara, tidak bisa dipakai lintas region — memakai Shopee di negara berbeda butuh app negara tersebut dan **nomor telepon yang terdaftar secara lokal** (akun `shopee.co.id` tidak bisa login ke `shopee.tw`, keduanya sistem akun yang sepenuhnya terpisah, bukan sekadar beda bahasa/tampilan dari platform yang sama). Karena penguji cuma punya nomor telepon/akun Indonesia, seluruh pengujian `AUTH_MODE=login` di bawah ini terpaksa dijalankan terhadap `shopee.co.id`, bukan target sebenarnya tugas ini (`shopee.tw`). Ini tidak melemahkan temuan secara arsitektur — platform yang sama dan pola `90309999` yang identik sudah terbukti bereplikasi lintas region (lihat catatan replikasi `.co.id` di atas) — tapi berarti hasil `AUTH_MODE=login` spesifik di bawah ini belum tervalidasi independen dengan akun `shopee.tw` asli.
...
**Ringkasan delapan variabel independen yang sudah dieliminasi**, semuanya menghasilkan `error: 90309999` yang identik:
...
| 7 | Device emulation (mobile vs desktop) | `DEVICE_EMULATION=mobile` (viewport, touch, UA iOS Safari) | Tetap gagal |
| 8 | Profil persisten vs snapshot `storageState` | `PERSISTENT_PROFILE=true` dengan login sungguhan langsung di dalam profil persisten baru (bukan cookies yang di-export/import) — lihat analisis "device-cookie binding" di bawah | Tetap gagal |
...
**Sudut yang masih genuinely belum diuji** dari kedua analisis ini, untuk eksperimen lanjutan kalau ada waktu/akses produk baru:
- **Rantai navigasi realistis penuh** (homepage → search → kategori → klik ke produk, bukan `page.goto()` langsung ke URL produk) dalam satu sesi persistent, dites lintas beberapa produk berurutan — berbeda dari teknik "warmup" kami saat ini yang cuma mampir ke homepage + delay acak sebelum tetap `goto()` langsung ke URL produk.

**Temuan tambahan — dua proyek scraper Shopee publik lain (GitHub, referensi: `dtungpka/shopee-scraper`, `toptankcpe/shopee-scraper`) ternyata beroperasi di layer yang sama sekali berbeda.** Keduanya pakai Selenium/SeleniumBase dengan login manual + captcha diselesaikan manusia, tapi **tidak pernah menyentuh `get_pc`/`get_rw` sebagai API terpisah** — mereka scrape DOM halaman yang sudah ter-render (nama, harga, review) via elemen visual, bukan intercept response JSON. Tidak ada satu pun yang menyebut kode error `90309999` atau strategi bypass anti-crawler spesifik di README mereka.

Untuk menguji apakah pendekatan ini bisa jadi jalan keluar, dicoba render penuh halaman produk (`shopee.co.id`, sesi login yang sama, produk yang sama) dan periksa: (a) HTML awal (SSR, sebelum JS jalan) untuk state ter-embed, dan (b) DOM setelah render penuh. Hasilnya nuansa penting: **HTML awal memang mengandung blob `initialState` dengan `shopId`/`itemId`, dan halaman berhasil render nama produk, gambar, rating, jumlah terjual, deskripsi lengkap, dan status stock — semua tampil normal secara visual.** Tapi **harga produk dan info shipping kosong** (placeholder abu-abu yang tidak pernah terisi) — persis di titik data yang bergantung pada `get_pc` yang gagal. `price_min`/`price_max` di blob `initialState` juga `null`.

Kesimpulan dari temuan ini: masalahnya **presisi** pada data yang bergantung pada `get_pc`, bukan kerusakan sesi/halaman secara luas — konten yang datang dari sumber lain (kemungkinan endpoint/CDN terpisah untuk metadata produk, gambar, deskripsi) tetap berhasil dimuat normal. Ini juga berarti pendekatan scrape-DOM (seperti dua proyek referensi di atas) **tidak akan memenuhi skema target tugas ini** (`get_pc.response_example.txt`, yang didominasi field harga/`shop_detailed`/`product_shipping`/`product_review`) — walau bisa dipakai untuk field non-komersial seperti judul/deskripsi/gambar, field paling krusial (harga, ongkir) tetap tidak tersedia lewat jalur ini.

**Catatan operasional penting — eskalasi risk-scoring pada akun uji.** Percobaan terakhir (navigasi via klik sungguhan: homepage → search → klik produk, bukan `goto()` langsung, untuk menguji sudut "rantai navigasi realistis" dari analisis eksternal) tidak sempat menguji hipotesisnya — navigasi ke homepage **langsung** di-redirect ke `/verify/captcha` (tantangan CAPTCHA sungguhan), bukan lagi `error: 90309999` diam-diam atau wall `/verify/traffic` seperti biasanya. Ini level eskalasi baru, kemungkinan besar akibat volume pengujian otomatis berulang terhadap akun/sesi yang sama selama sesi investigasi ini. Pengujian live lebih lanjut terhadap akun ini **dihentikan** pada titik ini untuk menghindari risiko lebih jauh terhadap akun pribadi penguji — konsisten dengan prinsip desain `TRAFFIC_VERIFICATION` di `retry.ts` (0 retry otomatis) yang sejak awal dibangun atas dasar kekhawatiran bahwa aktivitas berulang pasca-block cenderung memperparah, bukan memperbaiki, risk score.
...
Baris ketiga adalah celah nyata di antara dua percobaan sebelumnya — `PERSISTENT_PROFILE=true` sudah ada di `session.manager.ts`, tapi belum pernah benar-benar dites dengan login sungguhan di dalamnya dan dipakai berulang dari waktu ke waktu (bukan sekali pakai).

**Update — sudah diuji (konfirmasi ke-8): profil persisten sungguhan tetap gagal.** Profil Chrome dedicated (direktori profil baru & kosong — dijalankan pakai `patchright` untuk menghindari bug `playwright-extra` yang mengabaikan `executablePath` custom di `launchPersistentContext`) di-login manual sekali, langsung di dalam direktori profil itu (bukan lewat export/import `storageState` — sesi login hidup native di disk state profil itu sendiri). Lalu dipakai ulang lewat `PERSISTENT_PROFILE=true` terhadap produk `shopee.co.id` yang sama.

Hasil: **`get_pc` dan `get_rw` tetap mengembalikan `error: 90309999`.** Ini melemahkan kerangka "profil persisten vs snapshot storageState" dari hipotesis device-cookie-binding — setidaknya untuk profil yang *baru saja* dibuat, belum punya histori pemakaian nyata/"menua". Ada dua interpretasi yang masih terbuka: (a) pembedanya sama sekali bukan persisten-vs-snapshot, tapi histori pemakaian jangka panjang sungguhan yang belum dimiliki profil yang baru dibuat walau secara teknis sudah "persisten", atau (b) device-cookie binding bukan kerangka yang tepat, dan akar masalahnya tetap kontrol CDP/automation itu sendiri, terlepas dari bagaimana cookies disimpan. Mengingat kekhawatiran risiko akun yang sudah dicatat di atas, iterasi lanjutan untuk "menuakan" profil persisten dengan pemakaian nyata berjarak hari/minggu tidak dikejar di sesi ini.
...
### Terobosan: keberhasilan live sungguhan lewat rantai navigasi realistis penuh

Sekitar satu hari setelah eskalasi CAPTCHA yang dicatat di atas (cukup waktu untuk risk score akun mungkin mendingin), sudut "rantai navigasi realistis penuh" yang sempat terputus sebelumnya dicoba lagi — kali ini pakai profil persisten yang sama dari test di atas (sudah login, tidak lagi baru dibuat), dikombinasikan dengan **navigasi berbasis klik sungguhan, bukan `page.goto()`**: homepage → klik kotak pencarian → ketik kata kunci → tekan Enter → scroll → klik link produk asli di hasil pencarian, bukan membangun/navigasi langsung ke URL produk.
...
Meski begitu, ini hasil yang konkret dan reproducible, bukan sekadar dugaan — dan konsisten dengan dua sinyal independen yang sudah didokumentasikan di atas: saran analisis eksternal bahwa urutan/state navigasi berpengaruh (lihat "menyingkirkan hipotesis signature-replay" di atas), dan penekanan proyek referensi pada profil browser yang benar-benar dipercaya & "menua" (lihat "device-cookie binding" di atas). Implikasi praktis untuk strategi produksi: lebih baik mengendalikan front-end Shopee sendiri lewat navigasi berbasis klik yang realistis dari profil ter-otentikasi yang benar-benar dipakai lama, ketimbang deep-link langsung ke URL produk dari sesi baru — lebih dekat dengan cara kerja `shopee-mcp` dan dua scraper referensi berbasis Selenium, dan arsitektur yang cukup berbeda dari desain awal proyek ini (yang deep-link langsung lewat `page.goto()` ke `https://shopee.tw/a-i.{storeId}.{dealId}` demi kecepatan & kesederhanaan).

Keberhasilan ini terjadi di `shopee.co.id`, bukan target sebenarnya tugas ini (`shopee.tw`) — (lihat catatan sebelumnya soal alasannya: akun Shopee spesifik per region, cuma akun Indonesia yang tersedia) — jadi ini memvalidasi *pendekatannya*, bukan membuktikan berhasil di domain target spesifik. Mengingat kekhawatiran risiko akun yang sudah dibahas, akun/profil ini tidak didorong lebih jauh (mis. mengulang rantai ini langsung ke produk lain) untuk menghindari risiko eskalasi ulang.

**Mengisolasi variabel sebenarnya: klik vs `goto()`, dibuat deterministik & kompatibel API.** Run berhasil di atas masih punya ketidakcocokan struktural dengan kontrak API tugas ini: produknya didapat dengan **search organik lalu klik apa pun yang muncul pertama** — tapi API harus melayani `storeId`+`dealId` sembarang yang ditentukan caller, dan search Shopee cuma terima keyword, bukan ID produk. Jadi pertanyaan terbukanya: apakah "klik sungguhan" spesifik yang jadi pembeda (bukan proses search/discovery-nya, umur profil yang sedikit lebih "tua", atau waktu yang berlalu) — dan kalau iya, apakah bisa direproduksi untuk target **deterministik** yang sudah diketahui sebelumnya, bukan apa pun yang kebetulan muncul dari search.

Ini diuji langsung: ID produk baru dulu ditemukan lewat search **tanpa diklik** (cuma baca `href`-nya), lalu — di navigasi terpisah, reset kembali ke homepage — elemen `<a>` polos yang mengarah ke URL produk **spesifik yang sudah diketahui** itu disuntikkan ke DOM halaman via `page.evaluate()`, dan diklik pakai `.click()` Playwright (yang men-dispatch event input trusted di level CDP — kategorinya beda total dari `page.goto()`, yang lebih mirip mengetik URL langsung tanpa event apa pun di halaman).

**Hasil: berhasil lagi**, di produk baru kedua yang berbeda (`Powerbank Xm PLM02Z 10000 mAh`, `shopId=1651787050`, `itemId=44082009327`) — dipilih deterministik sebelumnya, bukan ditemukan lewat proses klik itu sendiri. Ini hasil kuncinya: **navigasi klik sungguhan ke produk sembarang yang sudah ditentukan sebelumnya berhasil, tanpa perlu search/discovery organik** — yang langsung kompatibel dengan kontrak API `GET /shopee?storeId=&dealId=`.

Ini sekarang diimplementasikan sebagai teknik resmi — `NAVIGATION_STRATEGY=click` (lihat `src/techniques/clickNavigation.ts`): mendarat di homepage, suntik link ke URL produk yang diminta persis, lalu klik link itu alih-alih `page.goto()` langsung ke sana. Sudah diuji terhadap sesi guest (tanpa login, tanpa profil persisten) menyasar domain asli tugas ini (`shopee.tw`) dan salah satu dari 2 item contoh asli yang sudah lama ter-flag berat — hasilnya tetap `error: 90309999`, tidak mengejutkan mengingat berapa kali item spesifik itu sudah "dihantam" sepanjang testing proyek ini (lihat "Metode & Eksperimen yang Dicoba" di atas).

**Update — percobaan replikasi lewat kode terintegrasi gagal, mengoreksi kerangka "klik saja sudah cukup".** Untuk memvalidasi teknik ini end-to-end (bukan cuma di skrip terpisah), teknik ini disambungkan ke `session.manager.ts` dan diuji lewat server live sungguhan, dites lewat URL publik ngrok — pertama ke produk baru yang belum pernah disentuh (`Apex Keyboard Mekanik Gaming`, `shopId=1744608100`, `itemId=46958749320`): **gagal**, `error: 90309999` di percobaan pertama. Langkah interaksi organik singkat kemudian ditambahkan (`warmupWithOrganicSearch()` di `clickNavigation.ts` — mengetik keyword pencarian acak yang tidak terkait dengan jitter, tekan Enter, tunggu hasil, scroll — sebelum klik suntikan), dengan alasan bahwa kedua keberhasilan sebelumnya selalu didahului interaksi search sungguhan di halaman yang sama tepat sebelum klik, sementara versi terintegrasi ini langsung dari homepage ke klik suntikan tanpa aktivitas apa pun sebelumnya. Diuji ulang ke produk baru kedua (`Robot Workmate III Mechanical Gaming Keyboard`, `shopId=131736292`, `itemId=40018658969`) dengan warmup organik terpasang: **gagal juga**, `error: 90309999` lagi.

**Interpretasi yang direvisi.** Baik "klik sungguhan" maupun "klik setelah interaksi organik" tidak berhasil mereproduksi keberhasilan sebelumnya saat dicoba ulang. Penjelasan paling masuk akal, mengingat kronologi lengkapnya: kedua keberhasilan terjadi dalam jendela waktu sempit tepat setelah eskalasi CAPTCHA akun sempat mendingin sekitar satu hari — dan setiap test otomatis yang dijalankan terhadap akun/profil yang sama sejak itu (termasuk kedua replikasi gagal ini, ditambah discovery script yang diselang-seling dan pengulangan ke item yang sudah terbakar) kemungkinan menambah sinyal mencurigakan ke akun spesifik itu, mendegradasi standing "bersih" apa pun yang menghasilkan kedua keberhasilan tadi, terlepas dari teknik navigasi apa yang dipakai. Dengan kata lain: keberhasilan itu mungkin mencerminkan **kondisi akun/sesi sementara** (risk score yang baru saja mendingin), bukan **teknik** yang reliable direproduksi kapan saja — click-navigation dan langkah warmup organik tetap ada di kode sebagai hal yang masuk akal & murah untuk dicoba (tidak ada bukti itu merugikan), tapi proyek ini belum bisa mengklaim itu *solusi*-nya, cuma bahwa keduanya hadir saat 2 keberhasilan nyata terjadi yang kondisinya belum bisa diisolasi bersih dari state akun saat itu.

Ini juga berarti **teknik ini belum divalidasi di `shopee.tw` sama sekali** (terhalang keterbatasan region-akun yang dibahas sepanjang dokumen ini) — yang terkonfirmasi adalah: dua keberhasilan nyata pernah terjadi sekali, di `shopee.co.id`, dalam kondisi yang proyek ini tidak bisa lagi direproduksi bersih di akun yang sama.

**Konfirmasi final — mengulang item yang sama persis yang berhasil sebelumnya juga gagal.** Sebagai pengecekan terakhir, item yang **persis sama** dengan yang berhasil di terobosan pertama (`GOOJODOQ Wireless Bluetooth 5.3 Headphones`, `shopId=1412347694`, `itemId=46755193498`) diminta ulang lewat server live, memakai ingredients identik yang menghasilkan keberhasilan awal — profil Chrome persisten yang sama & sudah login, engine `patchright` yang sama, strategi navigasi `direct` maupun `click` (lewat `NAVIGATION_STRATEGY=auto`). **Tetap gagal juga**, `error: 90309999` di setiap percobaan. Karena ini bukan kasus produk baru yang "terbakar" testing berulang (baru disentuh sekali, berhasil, sehari sebelumnya) dan setiap ingredient teknis identik dengan run yang berhasil, ini bukti paling kuat sejauh ini bahwa pembedanya bukan pernah teknik itu sendiri — melainkan kondisi risk-score akun/sesi pada momen spesifik itu, yang sejak itu sudah bergeser (kemungkinan besar makin terdegradasi oleh volume testing otomatis yang dijalankan proyek ini terhadap akun yang sama di antaranya, termasuk pengecekan konfirmasi ini sendiri). Mengingat risiko mendegradasi lebih jauh satu-satunya akun uji yang tersedia, tidak ada pengujian live lebih lanjut yang dikejar di akun ini setelah titik ini.
