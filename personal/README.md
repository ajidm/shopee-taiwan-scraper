...
**Why public Shopee scrapers mostly do DOM scraping (and why that's not really the key factor).**
Most public Shopee scraper repos found during research for this project (`dtungpka/shopee-scraper`,
`toptankcpe/shopee-scraper` — see the main README's "Known Limitations" section) scrape the
rendered page's DOM rather than intercepting `get_pc`/`get_rw` directly, which might suggest
DOM scraping is inherently safer against the anti-bot. Testing here points to a different
explanation: it's not the extraction method that matters, it's two things those tools happen
to require as part of their normal usage:

1. **A human manually logs in and solves any CAPTCHA before scraping starts** — both of
   those reference repos require exactly this. That's the same ingredient behind this
   project's own two real successes (a genuine, logged-in, persistent Chrome profile).
2. **Low request volume** — personal-use scrapers fetch a handful of products, not 200+ in
   a continuous run. They likely never trigger the same velocity-based degradation observed
   here (see the main README: the exact same successful item stopped working on retry, and
   general account/session risk visibly worsened with testing volume).

Direct evidence against "DOM is inherently safer": in the earlier SSR investigation
(documented in the main README), the `initialState` blob's price fields came back `null`
*during a blocked session* — not just the `get_pc` XHR call. So the SSR-embedded blob this
endpoint parses is generated by the same backend data-fetching logic and is subject to the
same anti-bot evaluation as `get_pc`; it isn't a separate, unguarded channel. When a session
is "let through," both methods return real data; when it isn't, both come back empty/null.
The reference scrapers' apparent reliability most likely comes from the human-verified
session + low volume, not from which endpoint the data happens to come from.
